/*
 * Copyright 2014 http://Bither.net
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package net.bither.util;

import android.graphics.Bitmap;
import android.graphics.Bitmap.Config;
import android.graphics.Canvas;
import android.graphics.Rect;
import android.util.Log;

/**
 * <p>
 * 图片Native压缩
 * </p>
 *
 * @author wenwei.chen
 * @version 1.0 (2015/12/6)
 */
public class NativeUtil {
    /**
     * 默认95
     */
    public static final int DEFAULT_QUALITY = 95;

    /**
     * 压缩
     *
     * @param bit      b
     * @param fileName 地址
     * @param optimize 优化
     */
    public static void compressBitmap(Bitmap bit, String fileName,
                                      boolean optimize) {
        compressBitmap(bit, DEFAULT_QUALITY, fileName, optimize);

    }

    /**
     * 压缩
     *
     * @param bit      b
     * @param quality  压缩比率（1-100）
     * @param fileName 地址
     * @param optimize 优化
     */
    public static void compressBitmap(Bitmap bit, int quality, String fileName,
                                      boolean optimize) {
        compressBitmap(bit,quality,fileName,optimize,3);
    }

    /**
     * 压缩
     *
     * @param bit      b
     * @param quality  压缩比率（1-100）
     * @param fileName 地址
     * @param optimize 优化
     * @param i 缩放比率
     */
    public static void compressBitmap(Bitmap bit, int quality, String fileName,
                                      boolean optimize,int i) {
        Log.d("native", "compress of native");
        Bitmap result = null;
        try {
            result = Bitmap.createBitmap(bit.getWidth() / i, bit.getHeight() / i, Config.ARGB_8888);// 缩小3倍
        } catch (OutOfMemoryError e1) {
            System.gc();
            try {
                i=6;
                result = Bitmap.createBitmap(bit.getWidth() / i, bit.getHeight() / i, Config.ARGB_8888);
            } catch (OutOfMemoryError e2) {
                System.gc();
                try {
                    i=10;
                    result = Bitmap.createBitmap(bit.getWidth() / i, bit.getHeight() / i, Config.ARGB_8888);
                } catch (OutOfMemoryError e3) {
                    System.gc();
                }
            }
        }
        Canvas canvas = new Canvas(result);
        Rect rect = new Rect(0, 0, bit.getWidth(), bit.getHeight());// original
        rect = new Rect(0, 0, bit.getWidth() / i, bit.getHeight() / i);// 缩小3倍
        canvas.drawBitmap(bit, null, rect, null);
        saveBitmap(result, quality, fileName, optimize);
        result.recycle();
    }

    private static void saveBitmap(Bitmap bit, int quality, String fileName,
                                   boolean optimize) {

        compressBitmap(bit, bit.getWidth(), bit.getHeight(), quality,
                fileName.getBytes(), optimize);

    }

    private static native String compressBitmap(Bitmap bit, int w, int h,
                                                int quality, byte[] fileNameBytes, boolean optimize);

    static {
        System.loadLibrary("jpegbither");
        System.loadLibrary("bitherjni");

    }

}
