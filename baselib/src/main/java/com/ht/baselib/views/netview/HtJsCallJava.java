...
/**
 * <p>JsCallJava类 JS调用原生函数管理对象</p>
 *
 * @author zhangguojun （from net https://github.com/seven456/SafeWebView
 *                      <br/>or http://www.pedant.cn/2014/07/04/webview-js-java-interface-research/）
 * @version 1.0
 *          1.1 update by zmingchun on 2015/6/21
 */
public class HtJsCallJava {
    private static final String TAG = HtJsCallJava.class.getSimpleName();
    private static final String RETURN_RESULT_FORMAT = "{\"code\": %d, \"result\": %s}";
    private static final String MSG_PROMPT_HEADER = "HtSafeWebview:";
     ...
     */
    public HtJsCallJava(Object interfaceObj, String interfaceName) {
        try {
            ...
            //拼接的js脚本格式参照备份文件：./doc/injected.js
            StringBuilder sb = new StringBuilder("javascript:(function(b){console.log(\"");
            sb.append(mInterfacedName);
            sb.append(" init begin\");var a={queue:[],callback:function()" +
                    "{var d=Array.prototype.slice.call(arguments,0);var c=d.shift();" +
                    "var e=d.shift();this.queue[c].apply(this,d);if(!e){delete this.queue[c]}}};");

            String sign;
            for (Method method : methods) {
                sign = genJavaMethodSign(method);
                if (sign == null) {
                    continue;
                }
            ...
            sb.append(mInterfacedName);
            sb.append(" call error, message:miss method name\"}var e=[];" +
                    "for(var h=1;h<f.length;h++){var c=f[h];var j=typeof c;e[e.length]=j;if(j==\"function\")" +
                    "{var d=a.queue.length;a.queue[d]=c;f[h]=d}}var k = new Date().getTime();var l = f.shift();var m=prompt('");
    ...
    private String genJavaMethodSign(Method method) {
        ...
        for (int k = 0; k < len; k++) {
            ...
            if (cls == String.class) {
                ...
                sign += "_O";
            } else if (cls == HtJsCallback.class) {
                sign += "_F";
            } else {
                sign += "_P";
            }
            ...
            try {
                ...
                for (int k = 0; k < len; k++) {
                    ...
                    if ("string".equals(currType)) {
                        ...
                        sign += "_F";
                        values[k] = new HtJsCallback(webView, mInterfacedName, argsVals.getInt(k));
                    } else {
                        sign += "_P";
                    }
                }
