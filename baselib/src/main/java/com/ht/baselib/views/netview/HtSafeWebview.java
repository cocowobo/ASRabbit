...
/**
 * <p>
 *     HtSafeWebview类 修补 webview 在Android4.2以下版本存在的注入提权漏洞
 * <br/>
 ...
 */
public class HtSafeWebview extends WebView {
    private static final String TAG = HtSafeWebview.class.getSimpleName();
    private Map<String, HtJsCallJava> mJsCallJavas;
     ...
     * @param context 上下文
     */
    public HtSafeWebview(Context context) {
        this(context, null);
    }
     ...
     * @param attrs 属性集
     */
    public HtSafeWebview(Context context, AttributeSet attrs) {
        super(context, attrs);
     ...
     * 3、在webChromeClient.onProgressChanged中都注入JS，并且不能通过自检查（onJsPrompt里面判断）JS是否注入成功来减少注入JS的次数
     * ，因为网页中的JS可以同时打开多个url导致无法控制检查的准确性；
     * 4、注入的JS中已经在脚本（./doc/notRepeat.js）中检查注入的对象是否已经存在，避免注入对象被重新赋值导致网页引用该对象的方法时发生异常；
     *
     * @deprecated Android4.2.2及以上版本的addJavascriptInterface方法已经解决了安全问题
     *             ，如果不使用“网页能将JS函数传到Java层”功能，不建议使用该类，毕竟系统的JS注入效率才是最高的；
     *
     * @param interfaceObj 注入的接口对象
     * @param interfaceName 注入的接口对象名称
     */
    @Override
    @Deprecated
    public void addJavascriptInterface(Object interfaceObj, String interfaceName) {
        if (mJsCallJavas == null) {
            mJsCallJavas = new HashMap<String, HtJsCallJava>();
        }
        mJsCallJavas.put(interfaceName, new HtJsCallJava(interfaceObj, interfaceName));
        setClient();
        if (mJsCallJavas != null) {
            injectJavaScript();
    ...
    public void destroy() {
        ...
        // ，解决办法很简单就是在Activity的ondestory方法中调用web.setVisibility(View.GONE);方法
        // ，手动将其隐藏，就不会崩溃了。在3.0一下系统上不会出现该崩溃问题，真是各种崩溃，防不胜防啊！
     ...
     * 注意：为了做到能100%注入，需要在注入的js中自行判断对象是否已经存在（如：if (typeof(window.HtAndroid) = 'undefined')）；
    ...
    private void injectJavaScript() {
        for (Map.Entry<String, HtJsCallJava> entry : mJsCallJavas.entrySet()) {
    ...
    public class SafeWebChromeClient extends WebChromeClient {
        ...
        public boolean onJsPrompt(WebView view, String url, String message, String defaultValue, JsPromptResult result) {
            if (mJsCallJavas != null && HtJsCallJava.isSafeWebViewCallMsg(message)) {
                JSONObject jsonObject = HtJsCallJava.getMsgJSONObject(message);
                String interfacedName = HtJsCallJava.getInterfacedName(jsonObject);
                if (interfacedName != null) {
                    HtJsCallJava jsCallJava = mJsCallJavas.get(interfacedName);
                    if (jsCallJava != null) {
    ...
    private void releaseConfigCallback() {
        if (Build.VERSION.SDK_INT < 16) { //Android 4.1 JELLY_BEAN 以下版本
            try {
        ...
        } else if (Build.VERSION.SDK_INT < 19) {//Android 4.4 KitKat 以下版本
            try {
