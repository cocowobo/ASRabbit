...
import com.ht.baselib.views.PullToRefreshView.ILoadingLayout.State;
 ...
 * @since 2015-11-17
 */
public class PullToZoomRefreshListView extends PullToRefreshBase<ListView> implements OnScrollListener {
     ...
     */
    public PullToZoomRefreshListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    /**
     * 设置滑动自动加载更多提前量,默认为滑动到底部自动加载更多
     * @param advanceNumItems 提前自动加载的item数
     */
    public void setAdvanceNumItems(int advanceNumItems) {
        this.advanceNumItems = advanceNumItems;
    }
    
    @Override
    protected boolean isReadyForPullUp() {
        return advanceNumItems <= 0 ? isLastItemVisible() : isAdvanceItemVisible();
    }
     ...
     * 判断是否到达提前量子项
     *
     * @return true到达提前量，否则false
     */
    private boolean isAdvanceItemVisible() {
        final Adapter adapter = mListView.getAdapter();
        ...
        return (lastItemPosition - lastVisiblePosition) <= advanceNumItems;
    }
